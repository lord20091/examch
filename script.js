...
alert(`agar ` + son1 + `ni ` + son2 + `ga ayirsa, qoldig'i ` + son1 % son2 + ` chiqadi`)
...
let RandomSon = Math.round(Math.random()*10)
...
alert(MARS + ` so'zida ` + MARS.length + `ta harif bor`)
...
function school(mars) {
    for (let i = 0; i < 10; i++) {
        console.log(mars);
    }
}

let it = `MARS IT SCHOOL`
...
console.log(harflar[0] + harflar[1] + harflar[2] + harflar[19] + harflar[10] + harflar[0] + harflar[23] + harflar[7] + harflar[23]);
...
if(yosh > 18) {
    ...
    alert(`siz balag'ot yoshiga yetmagansiz`)
} else {
    alert(`balag'ot yoshi muborak`)
}
...
for (const azz of sonlar) {
    if(azz % 2 == 0 ) {
        console.log(azz+`juft sonlar`);
    }
}